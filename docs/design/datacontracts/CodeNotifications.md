...
Batch callers (e.g., `IXCLRDataProcess::SetCodeNotifications`) do not receive an upfront
capacity check. If a batch would overflow the in-target table, the contract will write
as many entries as fit and then throw `COMException` with `HResult == E_FAIL` from the
first `SetCodeNotification` that cannot allocate a slot. This deliberately diverges from
the legacy DAC, which rejected oversize batches with `E_OUTOFMEMORY` before any writes.
